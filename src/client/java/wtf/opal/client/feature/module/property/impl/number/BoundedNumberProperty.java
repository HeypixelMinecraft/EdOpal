...
public final class BoundedNumberProperty extends Property<Pair<Double, Double>> {
    ...
    public void applyValue(JsonElement propertyValue) {
        if (propertyValue.isJsonObject()) {
            final JsonObject jsonProperty = propertyValue.getAsJsonObject();
            if (jsonProperty.isEmpty()) {
                return;
            }

            final double val1 = jsonProperty.get("x").getAsDouble();
            final double val2 = jsonProperty.get("y").getAsDouble();
