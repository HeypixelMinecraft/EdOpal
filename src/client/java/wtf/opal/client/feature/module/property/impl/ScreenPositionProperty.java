...
public final class ScreenPositionProperty extends Property<Pair<Float, Float>> {
    ...
    public void applyValue(JsonElement propertyValue) {
        if (propertyValue.isJsonObject()) {
            final JsonObject propertyObj = propertyValue.getAsJsonObject();
            if (propertyObj.isEmpty()) {
                return;
            }

            if (!propertyObj.has("x") || !propertyObj.has("y")) {
                return;
            }

            final float relativeX = propertyObj.get("x").getAsFloat();
            final float relativeY = propertyObj.get("y").getAsFloat();
