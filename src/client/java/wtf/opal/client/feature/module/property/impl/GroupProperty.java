...
public final class GroupProperty extends Property<List<Property<?>>> implements IPropertyListProvider {
    ...
    public void applyValue(JsonElement propertyValue) {
        if (propertyValue.isJsonArray()) {
            final JsonArray groupValues = propertyValue.getAsJsonArray();
            for (JsonElement jsonGroupPropElement : groupValues) {
                final JsonObject jsonGroupProp = jsonGroupPropElement.getAsJsonObject();
                final String groupName = jsonGroupProp.get("name").getAsString();
                final JsonElement groupValue = jsonGroupProp.get("value");
